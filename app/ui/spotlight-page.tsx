    ...
    <>
      ...
      <section className="special-content" aria-live="polite" aria-busy={loading}>
        {loading ? <div className="offseason-panel"><span className="offseason-mark" aria-hidden="true">🏈</span><div><h2>Checking the live schedule…</h2><p>Kickoff details are loading.</p></div></div> : slate.date ? (
          <><div className="special-answer"><div><p className="eyebrow">Next broadcast</p><h2>{formatLongDate(slate.date)}</h2></div><span>Times shown Eastern</span></div><div className="game-grid">{slate.games.map((game) => <GameCard key={game.id} game={game} />)}</div><div className="search-answer"><h2>{timeQuestions[kind]}</h2><p>{kind === "thanksgiving" ? `Thanksgiving kickoff times are ${slate.games.map((game) => formatKickoff(game.date).split(", ").slice(-1)[0]).join(", ")}. Channels are listed with each game above.` : `The next game starts at ${formatKickoff(slate.games[0].date).split(", ").slice(-1)[0]}. Watch on ${slate.games[0].broadcasts.join(", ") || "a channel to be announced"}.`}</p></div></>
