  ...
  return (
    <>
      ...
      {!loading && !failed && data ? <section className="team-content">
        ...
        <aside className="last-game-card"><p className="eyebrow">Last game</p>{data.lastGame && last ? <><div className={`result-pill result-${last.result.toLowerCase()}`}>{last.result}</div><h2>{last.score}</h2><p>{formatKickoff(data.lastGame.date)}</p></> : <><h2>No final yet</h2><p>The season’s first score is still waiting.</p></>}</aside>
      </section> : null}
